...
查找替换
/向下查找,?向上查找
  输入n 和 N 可以继续查找。 n是查找下一个内容,N查找上一个内容。
...
复制粘贴
普通模式中使用y复制:
	普通模式中，yy复制游标所在的整行（3yy表示向下复制3行）
	普通模式中，y^ 复制光标至行首，或y0。不含光标所在处字符。
	普通模式中，y$ 复制光标至行尾。含光标所在处字符。
	普通模式中，yw 复制光标开始的一个单词。
	普通模式中，y2w 复制两个单词。
	普通模式中，yG 复制至文本末。
	普通模式中，y1G 复制至文本开头。
...
hjkl：左下上右移动
20j：向下移动20行
w下一个单词的开头，e下一个单词的结尾,b到前一个单词开头 b返回上次的wore的过程
行间搜索：f + 字符，向后移动到某字符，按;向后继续查找，,向前查找，使用F是向前移动（vim基本所有字母的大小写功能都相反）继续搜索该行下一个/上一个
0或^：移动到行首，$移动到行尾
数字+gg:移动到第几行
